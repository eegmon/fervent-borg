  ...
  async (req, res) => {
    ...
    try {
      ...
      rows.forEach((r, i) => {
        ...
        const prosecutorId =
          prosecutorMap.get(prosecutorName) || prosecutorName;
...
// ── AI 공소사실 자동생성 / 교정 ───────────────────────────────────
// POST /api/ai/indictment-draft
// body: { mode: 'draft'|'refine', defendants, charges, incidentDate, caseNo, evidenceList, currentText }
// 응답: { success: true, result: string }
app.post(
  "/api/ai/indictment-draft",
  requireAuth,
  asyncWrap(async (req, res) => {
    ...
    if (!GEMINI_API_KEY) {
      return res
        .status(503)
        .json({ success: false, message: "AI 기능이 비활성화되어 있습니다. 서버에 GEMINI_API_KEY를 설정해주세요." });
    }

    const {
      mode = "draft",          // 'draft' | 'refine'
      defendants = [],
      charges = [],
      incidentDate = "",
      caseNo = "",
      evidenceList = [],
      currentText = "",        // refine 모드에서 원본 텍스트
    } = req.body;

    // ── 프롬프트 구성 ─────────────────────────────────────────────
    const defNames = defendants
      .map((d, i) => `피고인 ${i + 1}: ${d.name || "(성명 미상)"}${d.uuid ? ` (UUID: ${d.uuid})` : ""}`)
      .join(", ");
    const chargeNames = charges.map((c) => c.name).filter(Boolean).join(", ") || "해당 범죄";
    const lawArticles = charges.map((c) => c.lawArticle).filter(Boolean).join(", ");
...
5. 증거목록이 있으면 "관련 증거: OOO 등에 의하면" 형태로 자연스럽게 언급할 것`;
      ...
      if (!currentText.trim()) {
        return res.status(400).json({ success: false, message: "교정할 텍스트가 없습니다." });
      }
...
4. 마크다운, 특수문자, 제목 없이 순수 텍스트만 출력할 것`;
      ...
      return res.status(400).json({ success: false, message: "mode는 'draft' 또는 'refine'이어야 합니다." });
    }
    ...
    try {
      ...
      return res.status(502).json({ success: false, message: "AI 서버에 연결할 수 없습니다." });
    }

    if (!geminiRes.ok) {
      const errText = await geminiRes.text();
      console.error("[AI /indictment-draft] Gemini 오류:", geminiRes.status, errText);
      return res.status(502).json({ success: false, message: `AI 오류 (${geminiRes.status}): API 키를 확인해주세요.` });
    }
    ...
    if (!result) {
      return res.status(502).json({ success: false, message: "AI가 응답을 생성하지 못했습니다. 다시 시도해주세요." });
    }
