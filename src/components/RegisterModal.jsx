...
const AVAILABLE_ROLES = [
  { value: "PROSECUTOR_GENERAL", label: "검찰총장" },
  { value: "CHIEF_PROSECUTOR", label: "검사장" },
  { value: "DEPUTY_CHIEF", label: "차장검사" },
  { value: "SENIOR_PROSECUTOR", label: "부장검사" },
  { value: "PROSECUTOR", label: "평검사" },
  { value: "PROBATIONARY", label: "검사시보" },
  { value: "CHIEF_ADMINISTRATOR", label: "검찰관리관 (차장검사 대우)" },
  { value: "ADMINISTRATOR", label: "검찰사무관 (평검사 대우)" },
...
export default function RegisterModal({ isOpen, onClose, departmentsData }) {
  ...
  const validate = () => {
    ...
    if (form.password.length < 10) return "비밀번호는 10자 이상이어야 합니다.";
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      dept: form.dept,
      position:
        form.position.trim() || `${form.dept} ${selectedRole?.label || ""}`,
    ...
    <div className="modal-overlay">
      ...
      >
        ...
        >
          ...
          >
            <div>
              ...
              <div style={{ position: "relative" }}>
                <ChevronDown
                  size={15}
                  style={{ ...iconStyle, right: 10, left: "auto" }}
                />
                <select
          ...
          {/* 최초 관리자 초기화 코드 (선택) */}
          <div>
            <FieldLabel>최초 관리자 초기화 코드 (선택)</FieldLabel>
            <input
              className="input-field"
              type="password"
              placeholder="서버 운영자가 제공한 초기화 코드"
              value={form.bootstrapSecret}
              onChange={(e) => set("bootstrapSecret", e.target.value)}
              autoComplete="off"
            />
          </div>
          ...
          >
            ⚠️ 가입 신청 후 <strong>검찰사무국</strong>의 검토 및 허가를 받아야
            로그인이 가능합니다. 신청 결과는 관리자에게 문의해주세요.
          </div>
