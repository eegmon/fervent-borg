...
export default function DeadlineAlertModal({
  ...
  const alerts = useMemo(() => {
    ...
    appealsData.forEach(a => {
      if ((a.appealStatus || a.status || '').includes('접수') || (a.appealStatus || a.status || '').includes('심리')) {
        const baseDate = a.appealDate ? new Date(a.appealDate.replace(/\./g, '-')) : new Date();
    ...
    approvalsData.forEach(doc => {
      if ((doc.status || '').includes('대기') || (doc.status || '').includes('진행')) {
        list.push({
          id: `APPROVAL-${doc.id}`,
          category: 'APPROVAL',
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 16 }}>
      <div className="glass-panel gold-border" style={{ width: '100%', maxWidth: 760, maxHeight: '88vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRadius: 16 }}>
        {/* Header */}
        <div style={{ padding: '18px 24px', borderBottom: '1px solid var(--border-subtle)', background: 'var(--bg-elevated)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
        ...
        {/* Urgency Summary Bar */}
        <div style={{ padding: '12px 24px', background: 'rgba(245,158,11,0.06)', borderBottom: '1px solid rgba(245,158,11,0.15)', display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.8rem', fontWeight: 800, color: '#ef4444' }}>
            <ShieldAlert size={16} /> 긴급 처리 필요 ({criticalCount}건)
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.8rem', fontWeight: 700, color: '#f59e0b' }}>
            <AlertTriangle size={16} /> 주의 / 진행 경보 ({warningCount}건)
          </div>
          <div style={{ marginLeft: 'auto', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
            총 경보 알림: <strong style={{ color: 'var(--primary-amber)' }}>{alerts.length}건</strong>
          </div>
        </div>

        {/* Category Tabs */}
        <div style={{ padding: '10px 24px', display: 'flex', gap: 8, borderBottom: '1px solid var(--border-subtle)', background: 'var(--bg-card)' }}>
          {[
            { id: 'ALL', label: `전체 (${alerts.length})` },
            { id: 'ARREST', label: `🚨 구속 기한만료 (${alerts.filter(a => a.category === 'ARREST').length})` },
            { id: 'STATUTE', label: `⏳ 공소시효 (${alerts.filter(a => a.category === 'STATUTE').length})` },
            { id: 'APPEAL', label: `⚖️ 항고 심리기한 (${alerts.filter(a => a.category === 'APPEAL').length})` },
            { id: 'APPROVAL', label: `📄 미결재 (${alerts.filter(a => a.category === 'APPROVAL').length})` },
          ].map(t => (
            <button
              key={t.id}
              ...
              style={{
                ...
                background: activeTab === t.id ? 'var(--primary-amber)' : 'rgba(255,255,255,0.05)',
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    background: isCritical ? 'rgba(239,68,68,0.08)' : 'var(--bg-elevated)',
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1 }}>
                    ...
                    <div>
                      <div style={{ fontSize: '0.7rem', color: badgeColor, fontWeight: 800, marginBottom: 2 }}>
                        {item.categoryLabel}
                      </div>
                  ...
                  <button
                    ...
                    style={{ fontSize: '0.75rem', padding: '6px 12px', gap: 4, flexShrink: 0 }}
                  >
