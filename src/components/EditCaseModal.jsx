...
const parseDispositionMap = (value) => {
  if (!value) return {};
...
export default function EditCaseModal({
  ...
  const { hasDraft, readDraft, saveDraft, clearDraft } = useDraft(
    draftKey || "dose_draft_edit_none",
  );
  ...
  const isGlobalAccess = Boolean(
    currentUser?.isSuperAdmin || GLOBAL_DATA_ROLES.has(currentUser?.roleLevel),
  );
  ...
  const isProsecutorGeneral = Boolean(
    ...
    return [
      {
        ...
        role: "주범",
        bookingStatus: caseItem?.bookingStatus || "입건:불구속",
      },
    ];
  });
  const [suspectDispositions, setSuspectDispositions] = useState(() =>
    parseDispositionMap(caseItem?.suspectsDispositions),
  );
  ...
  useEffect(() => {
    ...
    setSuspectDispositions(parseDispositionMap(caseItem.suspectsDispositions));
    setSaveError(null);
  ...
  useEffect(() => {
    ...
    saveDraft({ formData, suspectsList, suspectDispositions });
  }, [formData, suspectsList, suspectDispositions, isOpen]);
  ...
  const handleRestoreDraft = () => {
    ...
    if (draft.suspectDispositions)
      setSuspectDispositions(draft.suspectDispositions);
  ...
  const getSuspectDisposition = (suspect) => {
    if (!suspect) return "";
    const key = suspect.id || suspect.uuid || suspect.name;
    return (
      suspectDispositions[key] ||
      (suspect.uuid && suspectDispositions[suspect.uuid]) ||
      (suspect.name && suspectDispositions[suspect.name]) ||
      (suspectsList.length === 1 &&
      DISPOSITION_OPTIONS.includes(formData.disposition)
        ? formData.disposition
        : "")
    );
  };

  const handleUpdateSuspectDisposition = (suspect, value) => {
    const keys = [suspect.id, suspect.uuid, suspect.name].filter(Boolean);
    setSuspectDispositions((prev) => {
      const next = { ...prev };
      keys.forEach((key) => {
        next[key] = value;
      });
  ...
  const handleSubmit = async (e) => {
    ...
    validSuspects.forEach((s, idx) => {
      const disposition =
        getSuspectDisposition(s) || formData.disposition || "";
      const key = s.id || s.uuid || s.name || `suspect-${idx}`;
      if (disposition) updatedSuspectsDispositions[key] = disposition;
    });
    const primaryDisposition =
      getSuspectDisposition(validSuspects[0]) || formData.disposition || "";
    ...
    const success = await onSave({
      ...
      bookingStatus:
        primarySuspect.bookingStatus || formData.bookingStatus || "",
        ...
        >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const selectedP = prosecutorsList.find(
          ...
          >
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ gridColumn: "1 / -1" }}>
                      ...
                      <select
                        className="select-field"
                        value={getSuspectDisposition(s)}
                        onChange={(e) =>
                          handleUpdateSuspectDisposition(s, e.target.value)
                        }
                      >
                        <option value="">처분 상태 선택</option>
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <ChargeSearchInput
                ...
                onChange={(v) =>
                  setFormData((prev) => ({ ...prev, chargeName: v }))
                }
            ...
            >
              ...
              <div>
                ...
                <input
                  className="input-field"
                  name="confiscation"
                  value={formData.confiscation || ""}
                  onChange={handleChange}
                  placeholder="예: 추징금 5,000,000원"
                />
              </div>
            </div>
          </div>
            ...
            >
              ...
              >
                ...
                <input
                  className="input-field"
                  name="court1No"
                  value={formData.court1No || ""}
                  onChange={handleChange}
                  placeholder="2026고단102"
          ...
          {canEditVisibility && (
            <div
              style={{
                ...
                borderRadius: 10,
                padding: 16,
                border: `1px solid ${formData.visibility === "PRIVATE" ? "rgba(239,68,68,0.35)" : "var(--border-subtle)"}`,
              }}
            >
              <div
                style={{
              ...
              {formData.visibility === "PRIVATE" && (
                <div style={{ marginTop: 12 }}>
                  <label className="input-label">
                    추가 열람 허용 검사 (검찰총장·담당검사·작성자는 자동 포함)
                  </label>
                  <select
                    className="select-field"
                    multiple
          ...
          <div>
            ...
            <textarea
              ...
              onChange={handleChange}
              placeholder="추가 메모 또는 수사 참고사항 기입..."
            />
          </div>
