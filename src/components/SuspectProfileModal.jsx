...
function StatCard({ label, value, color = "var(--primary-amber)" }) {
  return (
    ...
    >
      <div
        style={{ fontSize: "1.6rem", fontWeight: 900, color, lineHeight: 1 }}
      >
        {value ?? 0}
      </div>
      <div
...
export default function SuspectProfileModal({ uuid, onClose, currentUser }) {
  ...
  const avatarUrl = `https://crafthead.net/avatar/${uuid}?size=72&overlay=true`;
        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ flex: 1, overflowY: "auto" }}>
              {activeTab === "cases" && <CasesTab cases={profile.cases} />}
              {activeTab === "warrants" && (
              ...
              {activeTab === "stats" && <StatsTab stats={s} />}
            </div>
          </>
        ) : null}
      </div>
        ...
        >
          {[
            ...
            >
              ...
              <span style={{ color, fontWeight: 900, fontSize: "1.05rem" }}>
                {val ?? 0}
              </span>
            </div>
          ))}
...
function StatusBadge({ text }) {
  ...
  const isActive =
    text.includes("진행") || text.includes("청구") || text.includes("접수");
  const isDone =
    text.includes("완료") || text.includes("인용") || text.includes("발부");
  const isRejected =
    text.includes("기각") || text.includes("취소") || text.includes("실효");
  const bg = isDone
...
function DispositionBadge({ text }) {
  ...
  const isNonIndicted = ["불기소", "혐의없음", "기소유예", "각하"].some((k) =>
    text.includes(k),
  );
  const bg = isIndicted
    ? "rgba(239,68,68,0.1)"
