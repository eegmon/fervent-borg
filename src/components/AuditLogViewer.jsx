...
import {
  ClipboardList, Search, Filter, User, Clock,
  Trash2, CheckCircle, XCircle, Plus, Edit3, LogIn,
...
function parseDetail(detail) {
  ...
  const lines = detail.slice(idx + sep.length).split("\n").filter(Boolean);
  // 각 줄: "필드명: "이전값" → "이후값""
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 3 }}>
            <span style={{
              padding: "2px 7px", borderRadius: 5, fontSize: "0.71rem", fontWeight: 800,
              background: `${meta.color}20`, color: meta.color,
            }}>
              {meta.label}
            </span>
            <span style={{
              padding: "2px 7px", borderRadius: 5, fontSize: "0.71rem", fontWeight: 700,
            ...
            {hasChanges && (
              <span style={{
                padding: "2px 7px", borderRadius: 5, fontSize: "0.69rem", fontWeight: 700,
        ...
        <div style={{ flexShrink: 0, textAlign: "right", display: "flex", flexDirection: "column", gap: 3 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 4, justifyContent: "flex-end" }}>
            ...
            <span style={{ fontSize: "0.73rem", fontWeight: 700, color: "var(--text-main)" }}>
      ...
      {expanded && hasChanges && (
        <div style={{
          ...
          {changes.map((ch, i) =>
            ch.raw ? (
              <div key={i} style={{ fontSize: "0.74rem", color: "var(--text-muted)" }}>{ch.raw}</div>
            ) : (
              <div key={i} style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
                <span style={{
                  fontSize: "0.71rem", fontWeight: 700, color: "var(--text-muted)",
                ...
                <span style={{
                  fontSize: "0.74rem", color: "#4ade80",
                  background: "rgba(34,197,94,0.08)", padding: "1px 6px", borderRadius: 4,
                  fontFamily: "monospace", maxWidth: 200,
                  overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                  flexShrink: 0,
                }} title={ch.newVal}>
...
function HistoryRow({ item }) {
  return (
    <div style={{
      ...
      {/* 사건번호 */}
      <span style={{ fontFamily: "monospace", fontSize: "0.77rem", fontWeight: 700, color: "var(--primary-amber)", flexShrink: 0 }}>
      ...
      <span style={{
        fontSize: "0.74rem", color: "#f87171",
        background: "rgba(239,68,68,0.08)", padding: "1px 7px", borderRadius: 4,
        fontFamily: "monospace", maxWidth: 220,
        overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
        textDecoration: "line-through", opacity: 0.85, flexShrink: 0,
      }} title={item.oldValue}>
        {item.oldValue || "(없음)"}
      </span>
      <span style={{ fontSize: "0.68rem", color: "var(--text-muted)", flexShrink: 0 }}>→</span>
      ...
      <span style={{
        fontSize: "0.74rem", color: "#4ade80",
        background: "rgba(34,197,94,0.08)", padding: "1px 7px", borderRadius: 4,
        fontFamily: "monospace", maxWidth: 220,
        overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
        flexShrink: 0,
      }} title={item.newValue}>
      ...
      <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          <User size={11} color="var(--text-muted)" />
          <span style={{ fontSize: "0.71rem", fontWeight: 700, color: "var(--text-main)" }}>{item.actorName || item.actorId}</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          <Clock size={10} color="var(--text-muted)" />
          <span style={{ fontSize: "0.67rem", color: "var(--text-muted)", fontFamily: "monospace" }}>{item.createdAt}</span>
        </div>
      </div>
    </div>
  );
...
function CountBadge({ label, color, count, active, onClick }) {
  return (
    ...
    >
      <span style={{ fontSize: "0.77rem", fontWeight: 700, color }}>{label}</span>
      <span style={{ fontSize: "0.77rem", fontWeight: 800, color: "var(--text-main)" }}>{count}</span>
    </div>
  );
}
...
export default function AuditLogViewer({ auditLogs = [], caseHistory = [] }) {
  ...
  const hasFilter = search || filterAction !== "ALL" || filterEntity !== "ALL" || filterField !== "ALL";

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>

      {/* 헤더 */}
      <div className="glass-panel gold-border" style={{ padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
        <div>
        ...
        <div style={{ display: "flex", gap: 4, background: "var(--bg-card)", padding: 4, borderRadius: 10, border: "1px solid var(--border-subtle)" }}>
          <button style={TAB_STYLE(tab === "audit")} onClick={() => { setTab("audit"); resetFilters(); }}>
            <ClipboardList size={13} style={{ marginRight: 5, verticalAlign: "middle" }} />
            이벤트 로그
            <span style={{ marginLeft: 6, fontSize: "0.7rem", opacity: 0.8 }}>{auditLogs.length}</span>
          </button>
          <button style={TAB_STYLE(tab === "history")} onClick={() => { setTab("history"); resetFilters(); }}>
            <History size={13} style={{ marginRight: 5, verticalAlign: "middle" }} />
            원부 수정 이력
            <span style={{ marginLeft: 6, fontSize: "0.7rem", opacity: 0.8 }}>{caseHistory.length}</span>
          </button>
        </div>
      </div>
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
              ...
              <select className="select-field" value={filterEntity} onChange={(e) => setFilterEntity(e.target.value)} style={{ fontSize: "0.78rem" }}>
            ...
            {hasFilter && (
              <button onClick={resetFilters} className="btn btn-secondary" style={{ fontSize: "0.76rem", padding: "5px 11px" }}>
                초기화
              </button>
            )}
            <span style={{ fontSize: "0.73rem", color: "var(--text-muted)" }}>{filteredAudit.length}건 표시</span>
          </div>

          {/* 목록 */}
          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            {filteredAudit.length === 0 ? (
              <div style={{ padding: 48, textAlign: "center", color: "var(--text-muted)", fontSize: "0.84rem", background: "var(--bg-card)", borderRadius: 12 }}>
                감사 로그가 없습니다.
              </div>
            ) : (
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
              ...
              <select className="select-field" value={filterField} onChange={(e) => setFilterField(e.target.value)} style={{ fontSize: "0.78rem" }}>
                {fieldOptions.map((f) => (
                  <option key={f} value={f}>{f === "ALL" ? "전체 필드" : f}</option>
                ))}
              </select>
            </div>
            {hasFilter && (
            ...
            <span style={{ fontSize: "0.73rem", color: "var(--text-muted)" }}>{filteredHistory.length}건 표시</span>
          </div>
          ...
          <div style={{
            ...
            <span style={{ marginLeft: "auto", flexShrink: 0 }}>담당자 / 시각</span>
          </div>
