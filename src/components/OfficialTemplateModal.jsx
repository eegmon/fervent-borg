...
import { X, Copy, Check, Search, FileText, ChevronRight, Wand2, AlertCircle } from 'lucide-react';
...
function buildIframeDoc(template, caseItem, currentUser, today) {
  ...
  // 사건번호: "2025  0000호" 패턴
  body = body.replace(/20\d{2}(\s|&nbsp;){1,12}0+(\s|&nbsp;)*호/gi,
    `<strong style="color:#1e3a8a">${year}${caseNo}호</strong>`);

  // 날짜: ". . ." 패턴 (nbsp 사이에 점)
  body = body.replace(/((&nbsp;|\s){2,}\.\s*){3}/g,
...
export default function OfficialTemplateModal({
  isOpen, onClose, showToast, ledgerData = [], currentUser,
  // 결재 시스템 연동 (기안문용)
  ...
  const filtered = HWP_TEMPLATES.filter(t =>
    (category === 'ALL' || t.category === category) &&
    (!search || t.label.includes(search))
  );

  const selected   = HWP_TEMPLATES.find(t => t.id === selectedId) || HWP_TEMPLATES[0];
  const selectedCase = ledgerData.find(c => String(c.id) === String(selectedCaseId)) || null;
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      if (showToast) showToast('✅ 서식 복사 완료! 카페 스마트에디터에 Ctrl+V로 붙여넣기 하세요.', 'success');
    } catch (e) {
  ...
  const handleUseAsApproval = () => {
    ...
    onClose();
    if (showToast) showToast('📋 기안문이 결재 시스템에 전달되었습니다. 전자결재함에서 확인하세요.', 'success');
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderRadius: 16, width: '100%', maxWidth: 1280, height: '92vh',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>
                HWP 원본 양식 {HWP_TEMPLATES.length}종 · 사건 선택 시 자동입력 · 카페 Ctrl+V 붙여넣기
              </div>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          ...
          {/* ── 좌측: 서식 목록 ── */}
          <div style={{
            width: 270, flexShrink: 0, borderRight: '1px solid var(--border-subtle)',
            display: 'flex', flexDirection: 'column', overflow: 'hidden',
          }}>
            <div style={{ padding: '10px 12px 8px', borderBottom: '1px solid var(--border-subtle)' }}>
              <div style={{ position: 'relative' }}>
                ...
                <input
                  ...
                  style={{ paddingLeft: 30, fontSize: '0.77rem', padding: '7px 10px 7px 30px', width: '100%' }}
                  placeholder="서식 검색..."
                  value={search}
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 8 }}>
                {CATEGORIES.map(c => (
                  <button key={c.id} onClick={() => setCategory(c.id)} style={{
                    fontSize: '0.68rem', padding: '3px 9px', borderRadius: 10, cursor: 'pointer', fontWeight: 700,
                    background: category === c.id ? 'var(--primary-amber)' : 'var(--bg-elevated)',
                    color: category === c.id ? '#000' : 'var(--text-muted)',
                    border: category === c.id ? 'none' : '1px solid var(--border-subtle)',
                  }}>{c.label}</button>
                ))}
              </div>
            </div>
            <div style={{ flex: 1, overflowY: 'auto' }}>
              {filtered.map(t => (
                <button key={t.id} onClick={() => setSelectedId(t.id)} style={{
                  ...
                  background: selectedId === t.id ? 'rgba(245,158,11,0.1)' : 'transparent',
                  borderLeft: selectedId === t.id ? '3px solid var(--primary-amber)' : '3px solid transparent',
                ...
                >
                  <div style={{ fontSize: '0.76rem', fontWeight: selectedId === t.id ? 700 : 500, color: selectedId === t.id ? 'var(--primary-amber)' : 'var(--text-main)', lineHeight: 1.4 }}>
                    {t.no === 34 && <span style={{ fontSize: '0.65rem', background: 'rgba(59,130,246,0.2)', color: '#60a5fa', padding: '1px 6px', borderRadius: 8, marginRight: 4, fontWeight: 800 }}>결재</span>}
                    {t.label}
                  </div>
                </button>
              ))}
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{
              ...
              {!isGian && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontSize: '0.76rem', color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>사건 선택 →</span>
                  <select
                    className="select-field"
                    style={{ fontSize: '0.76rem', padding: '5px 10px', minWidth: 200, maxWidth: 320 }}
                  ...
                  {selectedCase && (
                    <span style={{ fontSize: '0.7rem', background: 'rgba(52,211,153,0.15)', color: '#34d399', padding: '2px 8px', borderRadius: 10, fontWeight: 700 }}>
                      ✨ 자동입력 적용
                    </span>
                  )}
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  {onCreateApprovalFromDoc && (
                    <button
                      ...
                      style={{ fontSize: '0.74rem', padding: '4px 12px', borderRadius: 8, cursor: 'pointer', background: 'rgba(59,130,246,0.2)', border: '1px solid rgba(59,130,246,0.4)', color: '#93c5fd', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 5 }}
                    >
              ...
              <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <button onClick={handleCopy} className="btn btn-gold" style={{ fontSize: '0.8rem', padding: '7px 16px', gap: 6, whiteSpace: 'nowrap' }}>
            ...
            {/* iframe 미리보기 */}
            <div style={{ flex: 1, overflow: 'hidden', background: '#e2e8f0', padding: 12 }}>
              <div style={{ height: '100%', background: '#fff', borderRadius: 8, overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
                <iframe
